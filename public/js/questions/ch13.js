window.CH13 = {
  ...
  description: "Operating activities section, adjustments to net income, gains/losses, and working capital changes",
  formulas: [
  ...
  generate: function() {
    ...
    var arBeg  = R(90200  * m);
    var arEnd  = R(83700  * m);   // AR decreased
    var arChange = arEnd - arBeg;  // negative => decrease

    var invBeg = R(102500 * m);
    var invEnd = R(112400 * m);   // Inventory increased
    var invChange = invEnd - invBeg;  // positive => increase

    var apBeg  = R(116500 * m);
    var apEnd  = R(65600  * m);   // AP decreased
    var apChange = apEnd - apBeg;  // negative => decrease

    var itpBeg = R(68600  * m);
    var itpEnd = R(50900  * m);   // ITP decreased
    var itpChange = itpEnd - itpBeg;  // negative => decrease
    ...
    var equipGain      = equipSalePrice - equipBookValue;   // positive => gain

    // Depreciation expense for the year
    //   ΔAccumDep on the books = AccumDep_end − AccumDep_beg
    //   But sold equipment took its AccumDep with it, so:
    //   Depreciation expense = (AccumDep_end − AccumDep_beg) + AccumDep_on_sold
    var deprExpense = (accumDepEnd - accumDepBeg) + equipAccumDep;

    // ===== Income & dividends =====
    // Target NI ~ $75k; back-calc beginning RE
    ...
    var adjGain = -equipGain;     // subtract gain
    var adjAR   = -arChange;       // decrease AR => +; increase AR => −
    var adjInv  = -invChange;
    var adjAP   = apChange;
    var adjITP  = itpChange;
    var combinedInvAP = adjInv + adjAP;
    ...
    // ===== MC builder =====
    // For amount + direction questions, we need both right amount and right direction
    ...
    // Precompute mc results (avoid double-call shuffle bug)
    ...
    var q12mc = mcSigned(adjAP, [-apChange, apEnd, -apEnd]);
    var q14mc = mcSigned(adjITP, [-itpChange, itpEnd, -itpEnd]);
    var q15mc = mcAmount(netOpCash, [netIncome, netIncome + adjDepr, netOpCash + adjGain * 2]);
    ...
    return {
      ...
      dataTable: [
        ...
        ["Cash dividends paid during the year", fmtPlain(dividends)],
      ...
      questions: [
        {
          ...
          steps: [{
            ...
            numbers: "Refer to the data table for net income and balance sheet items"
          }]
        },
        {
        ...
        },
        {
          ...
          steps: [{
            ...
            correct: 1,
            exp: "Depreciation reduced net income but no cash actually left the company. Adding it back removes its non-cash effect on net income.",
            result: "Treatment: Add depreciation back to net income",
            formula: "Add back non-cash expenses (Depreciation, Amortization)",
            numbers: "No numbers needed for this conceptual step"
          }]
        },
        {
          ...
          steps: [{
            ...
            exp: "Depr expense = (End Accum Dep − Beg Accum Dep) + Accum Dep removed on sold equipment = " + fmtPlain(accumDepEnd - accumDepBeg) + " + " + fmtPlain(equipAccumDep) + " = " + fmtPlain(deprExpense) + ".",
        ...
        },
        {
          ...
          steps: [{
            ...
            correct: 1,
            exp: "The full cash received from the sale appears in investing activities. The gain already inflated net income, so we subtract it from net income in operating to avoid double-counting.",
        ...
        },
        {
          ...
          steps: [{
            ...
            exp: "Book Value = " + fmtPlain(equipCost) + " − " + fmtPlain(equipAccumDep) + " = " + fmtPlain(equipBookValue) + ". Gain = " + fmtPlain(equipSalePrice) + " − " + fmtPlain(equipBookValue) + " = " + fmtPlain(equipGain) + " (gain because sale price > book value).",
        ...
        },
        {
          ...
          steps: [{
            inst: "Accounts receivable decreased during the year. How does this affect the reconciliation from net income to operating cash flow?",
            choices: [
              "A. Add the decrease to net income",
              "B. Subtract the decrease from net income",
              "C. No adjustment — only AR increases are relevant",
              "D. Report in investing activities"
            ],
            correct: 0,
            exp: "A decrease in a current asset means cash came in faster than the income statement showed. Add the decrease to net income.",
            result: "Direction: ADD the AR decrease to net income",
        ...
        },
        {
          ...
          steps: [{
            ...
            exp: "AR went from " + fmtPlain(arBeg) + " down to " + fmtPlain(arEnd) + ", a decrease of " + fmtPlain(arBeg - arEnd) + ". Decrease in current asset → ADD to net income.",
        ...
        },
        {
          ...
          steps: [{
            inst: "What does the accounts receivable decrease represent in cash terms?",
            choices: [
            ...
            correct: 1,
            exp: "AR going down means collections outpaced new credit sales — more cash came in than was reported as sales revenue. That extra cash is added back to net income.",
        ...
        {
          title: "Q10 — Inventory adjustment",
          steps: [{
            ...
            exp: "Inventory went from " + fmtPlain(invBeg) + " up to " + fmtPlain(invEnd) + ", an increase of " + fmtPlain(invEnd - invBeg) + ". Increase in current asset → SUBTRACT from net income.",
        ...
        {
          title: "Q11 — What the inventory change represents",
          steps: [{
            inst: "What does the inventory increase represent in cash terms?",
            choices: [
            ...
            correct: 1,
            exp: "Higher ending inventory means more was purchased than was sold (COGS). The extra purchases used cash that net income doesn't reflect, so we subtract the inventory build-up.",
            result: "Inventory increase means: Purchases > Cost of Goods Sold",
            formula: "Inventory increase → Purchases > COGS (subtract from NI)",
            numbers: "Refer to inventory balances in the data table"
          }]
        },
        {
          title: "Q12 — Accounts payable adjustment",
          steps: [{
            ...
            exp: "AP went from " + fmtPlain(apBeg) + " down to " + fmtPlain(apEnd) + ", a decrease of " + fmtPlain(apBeg - apEnd) + ". Decrease in current liability → SUBTRACT from net income.",
        ...
        {
          title: "Q13 — What the combined Inventory + AP adjustment represents",
          steps: [{
        ...
        {
          title: "Q14 — Income taxes payable adjustment",
          steps: [{
            ...
            exp: "Income taxes payable went from " + fmtPlain(itpBeg) + " down to " + fmtPlain(itpEnd) + ", a decrease of " + fmtPlain(itpBeg - itpEnd) + ". Decrease in current liability → SUBTRACT from net income. (Cash paid for taxes > tax expense.)",
        ...
        {
          title: "Q15 — Net cash provided by operating activities",
          steps: [{
            ...
            exp: "NI " + fmtPlain(netIncome) + " + Depr " + fmtPlain(adjDepr) + " − Gain " + fmtPlain(equipGain) + " + ΔAR " + (adjAR >= 0 ? "+" : "") + fmtPlain(adjAR) + " + ΔInv " + fmtPlain(adjInv) + " + ΔAP " + fmtPlain(adjAP) + " + ΔITP " + fmtPlain(adjITP) + " = " + fmtPlain(netOpCash) + ".",
